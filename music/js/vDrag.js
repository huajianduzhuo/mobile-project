...
//			}
		...
		wrap.addEventListener('touchmove', function (event) {
			...
			var scale = 1 - translateY / docHeight;
			
			if(translateY > 0){
				translateY = elemY + difY * scale;
			}else if(translateY < docHeight - list.offsetHeight){
				scale = 1 - (-translateY+docHeight-list.offsetHeight) / docHeight;
				translateY = elemY + difY * scale;
			}
		...
		wrap.addEventListener('touchend', function (event) {
			...
			if(targetY > 0){
				targetY = 0;
				type = 'easeOut';
			}else if(targetY < docHeight - list.offsetHeight){
				targetY = docHeight - list.offsetHeight;
				type = 'easeOut';
			}
			var time = 2;
